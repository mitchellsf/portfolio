...
The mass-spring-damper (MSD) system is a simplified mathematical model of many real-world systems. @fig-msd-general-schematic shows the basic system considered. The motion of the mass, $m$, as a function of time, $t$, is the objective of the problem. The mass is connected to the wall via a spring which provides a restoring force when stretched and a resisting force when compressed. This motion is opposed by a {{< ttip text="damper" title="A device which dissipates kinetic energy, usually via friction or fluid resistance." >}}.  In @fig-msd-general-schematic the damper is the friction on the wall as the mass moves through a pipe. 
...
$$ {#eq-gov-eq}
This equation is a second order, linear, inhomogeneous, ordinary differential equation (ODE) with constant coefficients. Let's define each of these terms.
...
A **homogeneous** ODE is linear but with no external forcing term. The external force, $f(t)$, in @eq-gov-eq makes the equation inhomogeneous. This distinction has important implications in solution strategies.
